...
void setup() {
    info = new InfoGrid(tileSize, xNumTiles, yNumTiles);
...
//   }
    if (mousePressed) {
        if (!isDragged) info.setPrevGrid();
        if (mouseButton == LEFT) info.setAtMouseLoc(true);
        if (mouseButton == RIGHT) info.setAtMouseLoc(false);
        isDragged = true;
    } else {
        isDragged = false;
    }
